...
def talker(gyro):
	pub = rospy.Publisher('gyro', String, queue_size=10)
...
while 1:
    ...
    if len(words) > 2:
        try:
            roll = float(gyro[1].replace(",", ""))*grad2rad
            pitch = float(gyro[3].replace(",", ""))*grad2rad
            yaw = float(gyro[5].replace(",", ""))*grad2rad
        except:
            print "Invalid line"
        ...
        #L1.text = str(float(citaj[2]))
        #L2.text = str(float(citaj[0]))
        #L3.text = str(float(citaj[4]))
ser.close
f.close
